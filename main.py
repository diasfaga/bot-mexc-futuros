...
RISK_PERCENT = float(os.getenv("RISK_PERCENT", 0.05))  # 5%
TP_PERCENT = float(os.getenv("TP_PERCENT", 0.01))      # 1%
SL_PERCENT = float(os.getenv("SL_PERCENT", 0.03))      # 3%
CANCEL_MINUTES = int(os.getenv("CANCEL_MINUTES", 5))

BASE_URL = f"https://api.telegram.org/bot{BOT_TOKEN}"
SYMBOLS = ['ALTUSDT', 'SAGAUSDT', 'ACEUSDT']  # adicione outros pares se quiser
...
def buscar_preco(symbol):
    try:
        ...
        print(f"Erro buscar_preco {symbol}: {e}")
        return None
...
def calcular_rsi(symbol, interval='5m'):
    try:
        ...
        print(f"Erro calcular_rsi {symbol}: {e}")
        return None
...
def processar_sinal(symbol):
    ...
    if rsi <= 30:
        ...
        qtd = round((valor * 5) / preco, 3)  # alavancagem 5x

        tp = round(preco * (1 + TP_PERCENT), 6)
        sl = round(preco * (1 - SL_PERCENT), 6)

        resp = enviar_ordem_limit(symbol, "1", qtd, preco)  # 1 = buy
